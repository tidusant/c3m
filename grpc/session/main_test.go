...
func TestSaveSession(t *testing.T) {
	...
	rs, err := svc.SaveSession(ctx, &pb.SessionMessage{Session: testsession, ShopID: shopTestId, UserID: UserTestId, UserName: UserNameTest})
	if err != nil {
		t.Fatalf("Test fail: %s", err.Error())
	}
	//check test data
...
func TestGetSession(t *testing.T) {
	...
	//check test data
	fmt.Printf("Data return: %+v\n", rs)
	if rs.ShopID != shopTestId || rs.UserID != UserTestId || rs.UserName != UserNameTest {
		t.Fatalf("Test Fail")
	}
}
